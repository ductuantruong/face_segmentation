...
def save_data_aug(data_dir, label_dir):
    for i, image in enumerate(listdir(data_dir)):
        ...
        cv2.imwrite(data_dir + str(n_total_file + 1) + '.jpg', bright_origin_img)
        cv2.imwrite(label_dir + str(n_total_file + 1) + '.png', label_img)        
        
        clahe_origin_img = A.CLAHE(p=1)(image=origin_img)['image']
        cv2.imwrite(data_dir + str(n_total_file + 2) + '.jpg', clahe_origin_img)
        cv2.imwrite(label_dir + str(n_total_file + 2) + '.png', label_img)   
        
        blur_origin_img = A.Blur(p=1)(image=origin_img)['image']
        cv2.imwrite(data_dir + str(n_total_file + 3) + '.jpg', blur_origin_img)
